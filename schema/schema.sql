...
CREATE TABLE `daily_stock_pick` (
  ...
  `sector_33_code_name` varchar(64) DEFAULT NULL COMMENT '33業種名。セクター分散上限の効き具合を後から検証するため保存',
  ...
  `return_1d` decimal(12,6) DEFAULT NULL COMMENT '1営業日後リターン（base_adj_close_price 基準）。未到来は NULL',
  `return_3d` decimal(12,6) DEFAULT NULL COMMENT '3営業日後リターン。未到来は NULL',
  `return_5d` decimal(12,6) DEFAULT NULL COMMENT '5営業日後リターン。未到来は NULL',
  `outcome` varchar(16) DEFAULT NULL COMMENT '勝敗判定: win/lose/draw/void。return_5d 基準',
  `evaluated_at` datetime DEFAULT NULL COMMENT '答え合わせ確定日時。NULL の行だけが答え合わせバッチの対象',
  `notified_at` datetime DEFAULT NULL COMMENT 'Slack 通知日時。NULL なら未通知＝次回バッチで再通知する',
  ...
  CONSTRAINT `daily_stock_pick_ibfk_1` FOREIGN KEY (`stock_brand_id`) REFERENCES `stock_brand` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
...
CREATE TABLE `fin_statement` (
  ...
  `forecast_dividend_per_share_annual` decimal(20,4) DEFAULT NULL COMMENT '1株あたり当期予想配当（年間合計）',
  `created_at` datetime NOT NULL,
  `updated_at` datetime NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `stock_brands_daily_price_for_analyze` (
  ...
  `ticker_symbol` varchar(36) NOT NULL COMMENT 'ticker symbol',
  `date` date NOT NULL COMMENT 'date',
  `open_price` decimal(14,4) NOT NULL COMMENT '始値',
  `close_price` decimal(14,4) NOT NULL COMMENT '終値',
  `high_price` decimal(14,4) NOT NULL COMMENT '高値',
  `low_price` decimal(14,4) NOT NULL COMMENT '安値',
  `adj_close_price` decimal(14,4) NOT NULL COMMENT '配当や株式分割を考慮した終値',
  `volume` bigint unsigned NOT NULL COMMENT '出来高',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'created_at',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'updated_at',
  PRIMARY KEY (`id`),
...
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `topix_daily_price` (
  `date` datetime NOT NULL COMMENT 'date',
  `open_price` decimal(10,4) NOT NULL COMMENT '始値',
  `close_price` decimal(10,4) NOT NULL COMMENT '終値',
  `high_price` decimal(10,4) NOT NULL COMMENT '高値',
